@echo off
REM ====================================================================
REM Git Commit and Push Script for Smart_Service
REM Features:
...
REM Change to Smart Service directory
cd /d "%~dp0\.."

REM Check if git repository exists in Smart Service folder only
if not exist ".git" (
    echo Error: Not a git repository in Smart_Service!
    echo Current directory: %CD%
    echo Smart_Service must have its own .git folder.
    exit /b 1
)

echo.
echo ==============================================================
echo   Git Commit and Push - Smart_Service
echo ==============================================================
echo.
...
REM Get git root (should be Smart_Service folder)
set GIT_ROOT=%CD%
...
REM ====================================================================
REM Step 2: Check Git Status (only Smart_Service files)
REM ====================================================================
cd /d "%GIT_ROOT%"
...
echo [Step 2/6] Checking git status for Smart_Service...
echo   Remote: !REMOTE_NAME! - !REMOTE_URL!
...
if !FILE_COUNT! equ 0 (
    echo   [WARNING] No changes in Smart_Service to commit
    del status.log 2>nul
    del error.log 2>nul
...
echo.
echo   [OK] Found !FILE_COUNT! file(s) with changes in Smart_Service
del status.log 2>nul
del error.log 2>nul
echo.

REM ====================================================================
REM Step 3: Add Changes (only Smart_Service)
REM ====================================================================
echo [Step 3/6] Adding Smart_Service changes...
echo   Remote: !REMOTE_NAME! - !REMOTE_URL!
echo   Branch: !TARGET_BRANCH!
if not "!TRACKING_REMOTE!"=="" (
    echo   Tracking: !TRACKING_REMOTE!/!TRACKING_BRANCH!
...
echo   [OK] Smart_Service changes added to staging
echo   Staged files:
...
git commit -m "[Smart_Service] !COMMIT_MESSAGE!" 2>error.log
if %ERRORLEVEL% neq 0 (
...
REM ====================================================================
echo ==============================================================
echo   Summary - Smart_Service
echo ==============================================================
echo   Service: Smart_Service
echo   Remote: !REMOTE_NAME! - !REMOTE_URL!
echo   Branch: !TARGET_BRANCH!
if not "!TRACKING_REMOTE!"=="" (
    echo   Tracking: !TRACKING_REMOTE!/!TRACKING_BRANCH!
)
echo   Commit: [Smart_Service] !COMMIT_MESSAGE!
echo ==============================================================
